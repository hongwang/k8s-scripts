...
class WorkerConfiguration(LoggingMixin):
    ...
    # changed start >>
    workspaces_volume_name = 'airflow-workspaces'
    # >> changed end
    ...
    def _get_volumes_and_mounts(self):
        ...
        volumes = {
            ...
            # changed start >>
            self.workspaces_volume_name: _construct_volume(
                self.workspaces_volume_name,
                "airflow-workspaces",
                None
            ),
            # << changed end
        ...
        volume_mounts = {
            ...
            # changed start >>
            self.workspaces_volume_name: {
                'name': self.workspaces_volume_name,
                'mountPath': self.kube_config.workspaces_volume_mountpath
            },
            # << changed end
            self.logs_volume_name: {
    ...
    def generate_dag_volume_mount_path(self):
        if self.kube_config.dags_volume_claim or self.kube_config.dags_volume_host:
            # changed start >>
            dag_volume_mount_path = self.kube_config.dags_volume_mountpath
            # << changed end
